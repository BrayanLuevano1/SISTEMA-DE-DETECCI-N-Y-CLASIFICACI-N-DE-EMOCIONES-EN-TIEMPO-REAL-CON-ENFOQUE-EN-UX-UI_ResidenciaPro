...
def make_mosaic(images, num_rows, num_cols, border=1, class_names=None):
    ...
    for image_arg in range(num_images):
        row, col = divmod(image_arg, num_cols)
        row_start, col_start = row * paddedh, col * paddedw
        mosaic[row_start:row_start + image_shape[0],
               col_start:col_start + image_shape[1]] = np.squeeze(images[image_arg])
    return mosaic
...
def make_mosaic_v2(images, num_mosaic_rows=None, num_mosaic_cols=None, border=1):
    ...
    num_mosaic_rows = num_mosaic_rows or box_size
    num_mosaic_cols = num_mosaic_cols or box_size
    ...
    mosaic_col_arg, mosaic_row_arg = 0, 0
    for image_arg in range(num_images):
        if image_arg % num_mosaic_cols == 0 and image_arg != 0:
            mosaic_col_arg += 1
            mosaic_row_arg = 0
        x0, x1 = image_pixels_cols * mosaic_row_arg, image_pixels_cols * (mosaic_row_arg + 1)
        y0, y1 = image_pixels_rows * mosaic_col_arg, image_pixels_rows * (mosaic_col_arg + 1)
        mosaic[y0:y1, x0:x1] = images[image_arg]
        mosaic_row_arg += 1
    return mosaic
...
def draw_mosaic(data, num_rows, num_cols, class_vectors=None, class_decoder=None, cmap='gray'):
    ...
    for ax, image, title in zip(axis_array.flat, data, titles):
