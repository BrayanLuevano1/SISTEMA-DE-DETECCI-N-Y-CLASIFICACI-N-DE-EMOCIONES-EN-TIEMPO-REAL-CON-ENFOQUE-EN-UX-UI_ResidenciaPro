...
def modify_backprop(model, name, task):
    ...
    with graph.gradient_override_map({'Relu': name}):
        # Reemplazar relu activation en todas las capas
        for layer in model.layers:
            if hasattr(layer, 'activation') and layer.activation == keras.activations.relu:
                layer.activation = tf.nn.relu

        # Re-instanciar un nuevo modelo
        if task == 'gender':
...
def deprocess_image(x):
    ...
    # Normalizar tensor: centrar en 0, asegurar que la desviación estándar sea 0.1
...
def compile_gradient_function(input_model, category_index, layer_name):
    ...
    # Capa lambda para la función de pérdida
    ...
    # Calcular la pérdida y obtener la salida de la capa deseada
    loss = K.sum(model.layers[-1].output)
    conv_output = model.layers[0].get_layer(layer_name).output

    # Normalizar y calcular gradientes
    gradients = normalize(K.gradients(loss, conv_output)[0])

    # Crear función para obtener salida de la capa y gradientes
